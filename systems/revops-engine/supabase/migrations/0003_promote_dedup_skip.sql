-- 0003: promote honors company dedup labels.
--
-- A staging company row labeled as a non-canonical duplicate (prep_dedup_kind in
-- exact_dup / acquired / alias, with prep_dedup_target set) is SKIPPED — it must not promote
-- alongside its canonical (e.g. FUJIFILM Diosynth must not land next to FUJIFILM Biotechnologies).
-- Hierarchy children (prep_dedup_kind = hierarchy) still promote — both parent and child are real.
-- Contacts have no prep_dedup_* columns, so the check is inert there.
--
-- Adds a 'deduped' count to the return (skipped duplicates). Return-type change -> DROP + CREATE.
...
) returns table(promoted int, inserted int, updated int, rejected int, deduped int, run_id uuid)
language plpgsql
as $fn$
declare
  v_staging_rel  text;
  ...
  v_sel_list     text;
  v_new_id       uuid;
  c_promoted int := 0; c_inserted int := 0; c_updated int := 0; c_rejected int := 0; c_deduped int := 0;
begin
  ...
  rejected := c_rejected; deduped := c_deduped; run_id := v_run_id; return next;
end;
$fn$;
