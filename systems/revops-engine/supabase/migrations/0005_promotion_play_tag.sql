-- 0005: tag each promotion with its play + verdict (for the play-segmented Records view).
--
-- The Records segment view filters canonical records to one play and shows In scope / Narrow.
-- That needs the play name and the verdict per promoted record. Neither rides onto the canonical
-- row (verdict is a staging label; a record can be in several plays), so we record both on the
-- promotion ledger (staging_promotions) at promote time. The segment view is then a clean join:
-- canonical record -> staging_promotions (by canonical_record_id) -> play_name + verdict.
--
-- Additive columns + backfill of play_name for existing rows. Function signature unchanged.

alter table public.staging_promotions add column if not exists play_name text;
alter table public.staging_promotions add column if not exists verdict text;
update public.staging_promotions sp
  set play_name = m.segment_name
  from public.staging_batch_meta m
  where m.batch_id = sp.batch_id and sp.play_name is null;
...
) returns table(promoted int, inserted int, updated int, rejected int, deduped int, skipped int, run_id uuid)
language plpgsql
as $fn$
declare
  v_staging_rel  text;
  ...
  v_sel_list     text;
  v_new_id       uuid;
  c_promoted int := 0; c_inserted int := 0; c_updated int := 0; c_rejected int := 0; c_deduped int := 0; c_skipped int := 0;
  v_play text;
begin
  ...
  select segment_name into v_play from public.staging_batch_meta where batch_id = p_batch_id limit 1;
  ...
  loop
    ...
    -- skip non-canonical duplicates (company dedup labels). Hierarchy children still promote.
    if coalesce(v_row->>'prep_dedup_kind','') in ('exact_dup','acquired','alias')
       and coalesce(v_row->>'prep_dedup_target','') <> '' then
      c_deduped := c_deduped + 1; continue;
    end if;

    -- verdict gate: only promote what the prep classification cleared. OUT / NEEDS_REVIEW
    -- (companies) and disqualified / out-of-scope / needs_review (contacts) stay in staging.
    if coalesce(v_row->>'prep_verdict','') in ('OUT','NEEDS_REVIEW')
       or coalesce(v_row->>'prep_contact_status','') in ('disqualified_company','out_of_scope_title','needs_review') then
    ...
    begin
      ...
      insert into public.staging_promotions
        (batch_id, source_record_type, source_record_id, canonical_record_id, enrichment_run_ids, promoted_by, promoted_at, play_name, verdict)
      values
        (p_batch_id, 'staging_'||(case p_entity when 'contacts' then 'contact' else 'company' end),
         ...
         v_play, coalesce(v_row->>'prep_verdict', v_row->>'prep_contact_status'));
  ...
  rejected := c_rejected; deduped := c_deduped; skipped := c_skipped; run_id := v_run_id; return next;
end;
$fn$;
