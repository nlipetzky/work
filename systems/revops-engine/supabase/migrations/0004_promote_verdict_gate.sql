-- 0004: promote honors the prep classification verdict (verdict-gating).
--
-- "Promote the IN set." Rows the classification excluded or could not verify must NOT land on
-- canonical (they stay in staging, flagged, not dropped):
--   companies: skip prep_verdict in ('OUT','NEEDS_REVIEW'); promote IN + NARROW.
--   contacts:  skip prep_contact_status in ('disqualified_company','out_of_scope_title',
--              'needs_review'); promote 'eligible'.
-- Rows with no prep label (pure-ingest batches) are ungated — promote as before.
-- Builds on 0003's dedup-skip. Adds a 'skipped' count. Return-type change -> DROP + CREATE.

drop function if exists public.promote_staging_batch(text, text, text);
