// =====================================================================
// L2 CLASSIFY — "Apply Rules" node — FULL REPLACEMENT BLOCK (R5+currency+trade-press, v5)
// 2026-05-19. Select-all -> paste this entire block into the Apply Rules
// Code node (mode: Run Once for All Items). Node must allow async (n8n
// Code node v2 supports top-level await).
//
// Change vs v4: trade-press signal precedence added.
// A program_status=ended signal from Perplexity (Company Events) overrides
// a stale CT.gov "active" verdict on the would-surface path. This closes
// the Pfizer/Adrenas gap. All other routes unchanged. 27-node topology
// (one new Read Trade Press Signals node added before this node).
// =====================================================================
...
const RULES_VERSION = 'revops-segment-aav-gene-therapy-ellie-outreach.md v4 R5+currency+trade-press v5';
...
// Build trade-press signal map from Read Trade Press Signals node (program_status from Perplexity)
// companyRecordId → {vitality, detectedAt, detail, sourceUrl}
const tradePressItems = $('Read Trade Press Signals').all();
const programStatusMap = {};
for (const sig of tradePressItems) {
  // Handle both Airtable response formats (fields nested or flat)
  const fields = (sig.json && sig.json.fields) ? sig.json.fields : sig.json;
  const company = fields['Company'];
  const cid = Array.isArray(company) ? company[0] : company;
  if (cid) {
    programStatusMap[cid] = {
      vitality: fields['Vitality'],
      detectedAt: fields['Detected At'],
...
const STALENESS_MS = STALENESS_YEARS * 365.25 * 24 * 3600 * 1000;
...
for (const c of candidates) {
  ...
  if (hitDisease.length > 0) { route='disease_reject'; routeIdx=1; verification='rejected'; reason = hitDisease.slice(0,3).join('; '); notes = nowIso + ' | v5 R5 | disease_aav_exclusion: ' + reason; customSrc = 'L2:disease_aav_exclusion'; }
  else if (hitModality) { route='modality_reroute'; routeIdx=2; verification='rejected'; custom = hitModality.mapsTo || hitModality.tok; const rr = rerouteMap[custom] || rerouteMap[hitModality.tok] || ''; reason = 'wrong_modality: ' + hitModality.tok; notes = nowIso + ' | v5 R5 | modality exclusion token "' + hitModality.tok + '"' + (rr ? (' | reroute: ' + rr) : ''); customSrc = 'L2:disqualifier_modality'; }
  else if (isDormant) { route='dormant'; routeIdx=3; verification='borderline'; reason='dormant: most recent trial >5yr, no active/recruiting (machine default, Q4 pending Ellie)'; customSrc='L2:dormancy_rule'; notes = nowIso + ' | v5 R5 | dormancy_rule: ' + (dormancyConfirmed.indexOf(nameN) !== -1 ? 'confirmed_dormant list' : 'most recent trial >5yr and not active/recruiting'); }
    ...
    if (ncts.length === 0) {
      ...
      notes = nowIso + ' | v5 R5 | canonical/intervention text match but NO CT.gov NCT stored to validate — manual AAV review';
    } else {
      ...
      if (v.verdict === 'Confirmed') {
        // Trade-press override: program_status=ended from Perplexity overrides stale CT.gov active
        const tpSignal = programStatusMap[id];
        let tradePressDone = false;
        if (tpSignal && tpSignal.vitality === 'ended') {
          const tpDate = tpSignal.detectedAt ? Date.parse(tpSignal.detectedAt) : Date.now();
          const withinWindow = (Date.now() - tpDate) <= STALENESS_MS;
          if (withinWindow) {
            route = 'needs_review'; routeIdx = 4; verification = 'borderline';
            reason = 'needs_aav_review'; customSrc = 'L2:trade_press_discontinued';
            currencyStatus = 'discontinued';
            currencyEvidence = 'Trade press (perplexity): program ended. ' + tpSignal.detail + (tpSignal.sourceUrl ? ' | ' + tpSignal.sourceUrl : '');
            currencyCheckedAt = nowIso.slice(0, 10);
            notes = nowIso + ' | v5 R5+currency+trade-press | ' + v.evidence + ' | TRADE PRESS OVERRIDE: program_status=ended | ' + tpSignal.detail;
            tradePressDone = true;
          }
        }
        if (!tradePressDone) {
          // No trade-press ended signal (or signal outside staleness window): derive from CT.gov
          ...
          if (cv.verdict === 'current') {
            ...
            notes = nowIso + ' | v5 R5+currency | ' + v.evidence + ' | ' + cv.evidence;
          } else {
            route='needs_review'; routeIdx=4; verification='borderline';
            reason='needs_aav_review'; customSrc='L2:r5_currency_gate';
            notes = nowIso + ' | v5 R5+currency | AAV confirmed but currency=' + cv.verdict + ' | ' + cv.evidence;
          }
        }
      } else {
        route='needs_review'; routeIdx=4; verification='borderline';
        reason='needs_aav_review'; customSrc='L2:r5_no_passing_trial';
        notes = nowIso + ' | v5 R5 | condition/intervention match but no cited trial passes R5 3-clause gate | ' + String(v.evidence).slice(0,400);
      }
    }
  }
  else if (brandedHit) { route='needs_review'; routeIdx=4; verification='borderline'; reason='needs_aav_review'; notes = nowIso + ' | v5 R5 | gene-therapy branded, no AAV terms (clause B) - needs AAV review'; customSrc='L2:clause_b_gene_therapy_branded_fallback'; }
  else { route='borderline'; routeIdx=5; verification='borderline'; notes = nowIso + ' | v5 R5 | no canonical match, no disease-AAV collision - manual review'; }
